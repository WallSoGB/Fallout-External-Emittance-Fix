...
public:
	...
	SKY_MODE						eMode;					// 0F8
	tList<SkySound>* pSkySoundList;			// 0FC
	float							fFlash;					// 100
	UInt32							uiFlashTime;			// 104
	UInt32							uiLastMoonPhaseUpdate;	// 108
	float							fWindowReflectionTimer;	// 10C
	float							fAccelBeginPct;			// 110
	UInt32							unk114;					// 114
	UInt32							uiFlags;				// 118
	ImageSpaceModifierInstanceForm* fadeInIMODCurrent;		// 11C
	ImageSpaceModifierInstanceForm* fadeOutIMODCurrent;		// 120
	ImageSpaceModifierInstanceForm* fadeInIMODLast;			// 124	On weather transition, set to the previuos weather fadeIn/OutIMODs
	ImageSpaceModifierInstanceForm* fadeOutIMODLast;		// 128		"			"
	float							f12_0;					// 12C	Always 12.0
	float							f23_99;					// 130	Always 23.99
	float							f0_0;					// 134	Always 0
